...
gulp.task('build', ['build-dev-client']);

gulp.task('build-release', ['build-release-client']);

gulp.task('build-dev-client', ['compile-lib', 'compile-client'], () => {
  return gulp
    .src('dist/client/after-dev.js')
    .pipe(
      webpack({
        quiet: true,
        output: { filename: 'after-dev.bundle.js' },
      })
    )
    .pipe(gulp.dest('dist/client'))
...
gulp.task('build-release-client', ['compile-lib', 'compile-client'], () => {
  return gulp
    .src('dist/client/after.js')
    .pipe(
      webpack({
        quiet: true,
        output: { filename: 'after.bundle.js' },
        plugins: [
...
gulp.task('watch', ['watch-bin', 'watch-lib', 'watch-server', 'watch-client']);
...
gulp.task('watch-lib', function() {
  return gulp.watch('lib/**/*.js', ['compile-lib', 'build-dev-client']);
});

gulp.task('watch-server', function() {
  return gulp.watch('server/**/*.js', ['compile-server']);
});
